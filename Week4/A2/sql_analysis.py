...
# ─────────────────────────────────────────────
# Import the library that need for this work
# ─────────────────────────────────────────────
...
import matplotlib.pyplot as plt     # use this for drawing the table as picture
...
#    put all setting in one place so easy to change later
# ═══════════════════════════════════════════════════════════════════════════════
...
class Config:
    ...
    # --- name for the SQLite database file going to create ---
    db_file: str = "world_happiness.db"


    # --- folder where save the output picture and CSV ---
    ...
    # --- colour for table header when draw the picture ---
...
#    keep all the SQL in one class so easy to find and maintain
# ═══════════════════════════════════════════════════════════════════════════════

class WorldHappinessQueries:
    """
    This class is hold the two SQL query that need to run.
    define them as class-level attribute so any part of script can access.
    ...
    #
    # Step A (gdp_grouped):
    #   label every country with GDP category using CASE statement.
    #    From GDP_per_Capita column:
    ...
    #   group by GDP_Category and calculate the average happiness score
    ...
    #   use RANK() window function with PARTITION BY GDP_Category.
    #   This mean ranking restart fresh for each category.
    #   Country with highest happiness score inside category get rank 1.
    #
    # Final SELECT:
    #   join country-level data with category average using GDP_Category.
    ...
    #   First calculate overall average corruption perception using subquery
    ...
    #   subtract that from group average to see how far above or below.
...
class DatabaseManager:
    ...
    use context manager pattern (with statement) internally
    so connection always close properly even if error happen.
    """

    def __init__(self, db_path: Path) -> None:
        # save the database path so can use later
    ...
    def load_csv_to_table(self, csv_path: Path, table_name: str = "world_happiness") -> pd.DataFrame:
        ...
        If table already exist, replace it so data is always fresh.

        Args:
            csv_path: where the CSV file is sitting
            table_name: what name give the table in SQLite

        Returns:
            The DataFrame that was loaded (useful for checking)
        """
        # first load the CSV into pandas DataFrame
        ...
        # strip that away so SQL query will not break
        ...
        # now connect to SQLite and push the DataFrame as a table
        # if_exists='replace' mean delete old table first, then create new
    ...
    def run_query(self, sql: str) -> pd.DataFrame:
        ...
        use read_sql_query from pandas because it automatically
        put column name and convert data type nicely.

        Args:
            sql: the SQL string want to execute
...
class ResultExporter:
    ...
    design it to be reusable — same exporter can handle any query result.
    """
    ...
    def __init__(self, output_dir: Path, config: Config) -> None:
        ...
        # also save the config so can use the colour setting
        self.config = config
    ...
    def save_csv(self, df: pd.DataFrame, filename: str) -> Path:
        ...
        index=False mean don't want the row number column in the file.
    ...
    def save_table_image(self, df: pd.DataFrame, title: str, filename: str) -> Path:
        ...
        calculate the figure size based on how many rows and column the data have
        so the table always fit properly in the image.

        Args:
            df: the DataFrame want to draw as table
        ...
        # only want to see the table, not the axis
        ax.axis("off")
        ...
        # now apply nice colour to each cell
...
class ConsolePrinter:
    ...
    separate printing logic from business logic so code is cleaner.
    """
...
class WorldHappinessAnalysis:
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        If no config given, use the default Config setting.
        """
        ...
        # each object has one job and combine them in this main class
        self.db_manager   = DatabaseManager(self.config.db_path)
        self.exporter     = ResultExporter(self.config.output_path, self.config)
        self.printer      = ConsolePrinter()
        self.queries      = WorldHappinessQueries()

        # placeholder for the query result, fill these during run()
    ...
    def _step_load_data(self) -> None:
        ...
        This is the first thing must do before running any query.
        """
        ...
        # the DataFrame is returned but only keep it for summary print
    ...
    def _step_run_query1(self) -> None:
        ...
        # print what got in terminal (show first 20 rows to not flood screen)
    ...
    def _step_export_results(self) -> None:
        ...
        save both format so the result is useful for report and analysis.
        """
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8. ENTRY POINT
#    This block only run when execute this file directly (not when imported).
